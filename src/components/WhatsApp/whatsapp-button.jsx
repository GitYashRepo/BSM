...
export default function WhatsAppWidget({ brandName = "Blush by Sakshi Makeover's", phone = '+919467777773' }) {
   const [open, setOpen] = React.useState(false)
   const [message, setMessage] = React.useState(
      `Hello ${brandName}, I would like to know more about your services. Can you please assist me?`,
   )
   ...
   return (
      <>
         ...
         <button
            ...
            onClick={() => setOpen((v) => !v)}
            className={cn(
               "fixed z-50 bottom-1 sm:bottom-2 right-1 size-12 rounded-full",
            ...
            >
               ...
               <header className="bg-[#075e54] text-white px-4 py-3 flex items-center gap-3 shrink-0">
                  <div className="size-8 rounded-full bg-white/15 grid place-items-center text-white font-semibold">
                     ...
                     <img src="/logo/logo.jpg" className="rounded-full" alt="Blush" />
                  </div>
               ...
               {/* Sticky Footer */}
               <footer className="px-3 py-2 bg-[#f6f7f5] border-t border-black/5 shrink-0 w-full">
                  <div className="flex items-center gap-2 bg-white rounded-full px-4 py-2 shadow-sm min-w-0 overflow-hidden w-full">
                     <input
                        className="flex-1 w-full min-w-0 bg-transparent outline-none text-sm placeholder:text-[#667085]"
                     ...
                     <button
                        type="button"
                        className="size-9 rounded-full hover:cursor-pointer bg-[#25d366] text-white grid place-items-center shrink-0"
